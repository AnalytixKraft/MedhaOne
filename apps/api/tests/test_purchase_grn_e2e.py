...
def test_purchase_to_grn_end_to_end(client_with_test_db: tuple[TestClient, Session]) -> None:
    ...
    approve_resp = client.post(f"/purchase/po/{po_id}/approve", headers=headers)
    ...
    post_resp = client.post(f"/purchase/grn/{grn_id}/post", headers=headers)
...
def test_grn_over_receipt_blocked(client_with_test_db: tuple[TestClient, Session]) -> None:
    ...
    po_line_id = po_resp.json()["lines"][0]["id"]
    ...
    assert "exceeds remaining quantity" in grn_resp.json()["detail"]
...
def test_grn_double_post_protection(client_with_test_db: tuple[TestClient, Session]) -> None:
    ...
    grn_number = grn_resp.json()["grn_number"]

    first_post = client.post(f"/purchase/grn/{grn_id}/post", headers=headers)
    assert first_post.status_code == 200, first_post.text

    second_post = client.post(f"/purchase/grn/{grn_id}/post", headers=headers)
    assert second_post.status_code in (400, 409)
