...
export type GlobalTaxRateRecord = {
  id: number;
  code: string;
  label: string;
  ratePercent: string;
...
export async function listGlobalTaxRates() {
  return withPgClient(async (client) => {
    ...
    const result = await client.query<GlobalTaxRateRow>(
      `SELECT id, code, label, rate_percent::text, is_active, created_at::text, updated_at::text
...
export async function createGlobalTaxRate(rawInput: unknown) {
    ...
    try {
      const result = await client.query<GlobalTaxRateRow>(
        `INSERT INTO public.global_tax_rates (code, label, rate_percent, is_active)
         VALUES ($1, $2, $3, $4)
         RETURNING id, code, label, rate_percent::text, is_active, created_at::text, updated_at::text`,
        [input.code, input.label.trim(), input.ratePercent.toFixed(2), input.isActive],
      );
...
export async function updateGlobalTaxRate(id: number, rawInput: unknown) {
  const input = updateGlobalTaxRateInput.parse(rawInput);

  return withPgClient(async (client) => {
    await ensureGlobalTaxRatesTableAndDefaults(client);
    const existing = await client.query<{ id: number }>(
      "SELECT id FROM public.global_tax_rates WHERE id = $1",
      [id],
    );
    if (!existing.rows[0]) {
    ...
    if (updates.length === 0) {
      const unchanged = await client.query<GlobalTaxRateRow>(
        `SELECT id, code, label, rate_percent::text, is_active, created_at::text, updated_at::text
         FROM public.global_tax_rates
         WHERE id = $1`,
        [id],
    ...
    try {
      const result = await client.query<GlobalTaxRateRow>(
        `UPDATE public.global_tax_rates
...
export async function seedTenantTaxRatesFromGlobalDefaults(client: PoolClient, schemaName: string) {
  await ensureGlobalTaxRatesTableAndDefaults(client);
  const schema = quoteIdentifier(schemaName);
  await client.query(
    `INSERT INTO ${schema}.tax_rates (code, label, rate_percent, is_active)
...
async function ensureGlobalTaxRatesTableAndDefaults(client: PoolClient) {
  await client.query(`
  ...
  `);
  await client.query(
    "CREATE INDEX IF NOT EXISTS idx_global_tax_rates_rate_percent ON public.global_tax_rates(rate_percent)",
  );
  await client.query(
    "CREATE INDEX IF NOT EXISTS idx_global_tax_rates_is_active ON public.global_tax_rates(is_active)",
  );
  ...
  for (const taxRate of defaultGlobalTaxRates) {
    await client.query(
      `INSERT INTO public.global_tax_rates (code, label, rate_percent, is_active)
       VALUES ($1, $2, $3, TRUE)
       ON CONFLICT (code) DO NOTHING`,
      [taxRate.code, taxRate.label, taxRate.ratePercent],
    );
  }
}

function mapGlobalTaxRateRow(row: GlobalTaxRateRow): GlobalTaxRateRecord {
  return {
    id: row.id,
    code: row.code,
    label: row.label,
