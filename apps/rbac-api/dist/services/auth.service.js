...
export async function seedSuperAdminIfMissing(email, passwordHash) {
...
export async function login(rawInput) {
    ...
    if (!input.organizationId) {
        ...
        if (matches.length > 1) {
            throw new AppError(409, "ORG_SELECTION_REQUIRED", "Multiple organizations found for this account", {
...
export async function createSudoToken(superAdminId, organizationId) {
    const organization = await prisma.organization.findUniqueOrThrow({ where: { id: organizationId } });
    return withPgClient(async (client) => {
        await client.query("BEGIN");
        try {
            await client.query(`SET LOCAL search_path TO ${organization.schemaName}, public`);
            const result = await client.query(`SELECT id, email, full_name FROM users WHERE role = 'ORG_ADMIN' AND is_active = TRUE ORDER BY created_at ASC LIMIT 1`);
...
async function findTenantMatches(email, password) {
...
async function findTenantMatchInOrganization(organization, email, password) {
    if (!organization.isActive) {
        return null;
    }
    return withPgClient(async (client) => {
        const result = await client.query(`SELECT id, email, password_hash, full_name, role, is_active
...
async function issueTenantLogin(match) {
    return withPgClient(async (client) => {
        await client.query("BEGIN");
        try {
