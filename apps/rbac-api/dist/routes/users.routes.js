...
import { createOrgUser, listOrgUsers, setOrgUserActive, updateOrgUserRole, } from "../services/user.service.js";
...
usersRouter.post("/", requireRoles("ORG_ADMIN"), async (req, res, next) => {
    try {
        const user = await createOrgUser(req.auth.userId, req.auth.organizationId, req.tenantSchema, req.body);
...
usersRouter.patch("/:userId/status", requireRoles("ORG_ADMIN"), async (req, res, next) => {
    try {
        ...
        const { userId } = req.params;
        const user = await setOrgUserActive(req.auth.userId, req.tenantSchema, userId, body.isActive);
        res.json(user);
    }
    catch (error) {
        next(error);
